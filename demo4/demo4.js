...
function make_model_runner ( params ) {
  ...
  function next (target) {
    ...
    return { position : P, velocity : V, force : F, e3: pos_err };
  }
...
window.onload = function () {
  ...
  s1 = make_slider (sim.model_system, 'Kp', {x: 50, y:50, min:0, max:20, step:0.01, tooltip:"Position gain"});
  s2 = make_slider (sim.model_system, 'Kv', {x: 50, y:90, min:0, max:500, step:1, tooltip:"Velocity gain"});
  s3 = make_slider (sim.model_system, 'M', {x: 50, y:130, min:0.2, max:20, step:0.01, tooltip:"Mass"});
  s4 = make_slider (sim.model_system, 'Ks', {x: 50, y:170, min:0, max:250, step:1, tooltip:"Spring constant"});
  s5 = make_slider (sim.model_system, 'Kd', {x: 50, y:210, min:0, max:100, step:0.01, tooltip:"Damping constant"});
  s6 = make_slider (sim.model_system, 'pos_ref', {x: 50, y:250, min:-100, max:100, step:0.1, text:"Target"});
  ...
  var prop_load = make_bode_plot({x:40, y: 600});
  var single_integral_load = make_bode_plot({x:260, y: 600})
  var double_integral_load = make_bode_plot({x:480, y: 600})
  ...
  function run_model ( len ) {
    ...
    var i =  0;
    var result, target;
    var data = { position : [], force : [], velocity: [], pos_ref : []};
  ...
  pl = svg_plot("plot", run_model(6000));

  function plot_model () {
    pl.new_plot(run_model(6000));
  }
...
function frequency_analysis () {
  ...
  return { prop_load          : { gain: bode_plots_gain[0],
                                  phase: bode_plots_phase[0] },
           integral_load      : { gain: bode_plots_gain[1],
                                  phase: bode_plots_phase[1] },
          double_integral_load: { gain: bode_plots_gain[2],
...
function svg_plot (plot_area, data, clrs) {
  ...
  var i, k;
  var elem = [];
  clrs = clrs || {'force':'blue', position:'red', velocity:'green', pos_ref:'gray'};
