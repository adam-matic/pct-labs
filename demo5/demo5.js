...
function init () {
  ...
  var size  = 25, origin = { x:150, y:150 };
  ...
  var it = {x : origin.x,
           ...
           h : size,
           text:"I"};
  var ic = {x : origin.x + size * gr * 1.2,
           y : origin.y,
           w : size * gr,
           h : size,
           text:"C"};
  var io = {x : origin.x + size*gr*2*1.2,
           y : origin.y + size*1.3,
           w : size * gr,
           h : size,
            text:"O"};
  ...
  [i, c, o].map(function (x) { mb.appendChild(x.box) });



  var ref = make_box().init({x:origin.x, y:origin.y, w:size*gr, h:size, text:"0.00"});
